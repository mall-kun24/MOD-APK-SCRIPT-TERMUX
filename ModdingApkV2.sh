...
while true; do
    ...
    # =============================================================
    # MENU MODIFIKASI FITUR
    # =============================================================
    echo -e "\n${CYAN}PILIH FITUR MODIFIKASI:${RESET}"
    echo -e "  ${GREEN}[1] Unlock VIP / Premium / Pro (Injeksi Hook VIP)${RESET}"
    ...
    if [ "$EXEC_METHOD" -eq 1 ]; then
        ...
        # Injeksi Hook / Remove Ads
        (
            if [ "$MOD_CHOICE" -eq 1 ] || [ "$MOD_CHOICE" -eq 3 ]; then
                NEW_SMALI_DIR="$WORK_DIR/smali_classes2"
                mkdir -p "$NEW_SMALI_DIR/com/modder/bypass"

                cat << 'EOF' > "$NEW_SMALI_DIR/com/modder/bypass/VipBypass.smali"
...
EOF
        ...
        if [ "$MOD_CHOICE" -eq 1 ]; then
            show_progress "Menyuntikkan Modul Hook Premium / VIP / Pro" "$PID"
        elif [ "$MOD_CHOICE" -eq 2 ]; then
            show_progress "Menjalankan Pembersihan Iklan & Manifest Ads" "$PID"
        elif [ "$MOD_CHOICE" -eq 3 ]; then
            show_progress "Menyuntikkan Hook VIP/Pro & Pembersihan Iklan" "$PID"
        else
        ...
        # ---------------------------------------------------------
        # METODE 2: LANGSUNG DARI APK (DIRECT APK INJECTION)
        # ---------------------------------------------------------
        echo -e "${CYAN}[*] Memproses modifikasi langsung pada file APK...${RESET}"
        
        DIRECT_TEMP="direct_apk_temp"
        rm -rf "$DIRECT_TEMP" "$UNALIGNED_APK"
        mkdir -p "$DIRECT_TEMP"

        (
            # Ekstrak seluruh isi APK ke direktori sementara
            unzip -q "$TARGET_APK" -d "$DIRECT_TEMP" 2>/dev/null

            # Hapus folder arsitektur lib yang dipilih pengguna
            if [ -d "$DIRECT_TEMP/lib" ] && [ ${#ARCHS_TO_REMOVE[@]} -gt 0 ]; then
                for DEL_ARCH in "${ARCHS_TO_REMOVE[@]}"; do
                    rm -rf "$DIRECT_TEMP/lib/$DEL_ARCH"
                done
            fi

            # Re-pack menjadi APK baru agar perubahan ter-update secara menyeluruh
            cd "$DIRECT_TEMP"
            zip -qr "../$UNALIGNED_APK" ./* 2>/dev/null
            cd ..
            rm -rf "$DIRECT_TEMP"
        ) &
        PID=$!
        show_progress "Memproses Pembersihan Folder Lib pada APK" "$PID"
        wait $PID
    fi
    ...
    if [ "$COMPRESS_CHOICE" -eq 1 ]; then
        ...
        (
            ...
            cd "$TEMP_ULTRA_DIR"
            # Kompres level 9 seluruh isi KECUALI resources.arsc
            zip -9 -qr "../$ULTRA_COMPRESSED_APK" ./* -x "resources.arsc" 2>/dev/null
            # Tambahkan resources.arsc tanpa kompresi (store mode -0) agar tidak crash di Android
        ...
        # --- OPSI 3: TANPA KOMPRESI (STORE LEVEL 0) ---
        ...
        (
            ...
            cd "$TEMP_STORE_DIR"
            # Paksa buat ulang zip dengan level kompresi 0 (Store Only / Uncompressed)
